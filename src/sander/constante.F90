...
!              It was adapted from the constantph module initialy written on Sander by John Mongan.
!
...
_REAL_, public  :: chrgdat_e(0:ATOM_CHRG_C-1)
...
_REAL_  :: eo_corr(0:TITR_STATES_C-1)
...
#  include "../include/md.h"
   ...
   if (trescnt > TITR_RES_C) then
      write(6,*) 'Too many titrating residues; alter dyne.h and recompile'
      call mexit(6,1)
   end if
   do itres = 0, trescnt - 1
   ...
   if (icumstat > TITR_STATES_C) then
      write(6,*) 'Too many titrating states; alter dyne, recompile'
      call mexit(6,1)
   end if
   if (icumchrg > ATOM_CHRG_C) then
      write(6,*) 'Too much charge data; alter dyne.h, recompile'
      call mexit(6,1)
   end if
...
subroutine cnstewriterestart(inchrgdat_e)
   use constants, only : AMBER_ELECTROSTATIC
   use file_io_dat, only : CNSTE_UNIT, cerestrt, owrite, CEOUT_UNIT
   implicit none
...
#  include "../include/md.h"
   ...
   if (full) then
      ...
      write(CEOUT_UNIT, '(a,f10.3)') 'Time: ', t
      do i = 0, trescnt - 1
