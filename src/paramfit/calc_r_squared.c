      ...
      if (global_options->FUNC_TO_FIT==SUM_SQUARES_AMBER_STANDARD) {
        ...
        for (atom=0; atom<coords_data[i].natoms; ++atom) {
          if (parm_datas[i].fit_atom[atom] == YES) {
            x_value += fabs(single_eval[atom].x) + fabs(single_eval[atom].y) + fabs(single_eval[atom].z);
            y_value += fabs(coords_data[i].struc[point].force[atom].x) + fabs(coords_data[i].struc[point].force[atom].y) + fabs(coords_data[i].struc[point].force[atom].z);
          }
        }
        free(single_eval);
      }
