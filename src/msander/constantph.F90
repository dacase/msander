...
_REAL_  :: pka_corr(0:TITR_STATES_C-1)
...
subroutine cnstphwriterestart(inchrgdat)
   ...
   use file_io_dat, only : CNSTPH_UNIT, cprestrt, cperestrt, owrite, CPOUT_UNIT
   implicit none
   ...
   if (first_cprestrt) then
      ...
      if (.not. cpein_specified) then
        ...
        open(unit=CNSTPH_UNIT, file=cperestrt, status=stat, form='FORMATTED', &
             delim='APOSTROPHE')
      end if
      ...
      if (.not. cpein_specified) then
        ...
        open(unit=CNSTPH_UNIT, file=cperestrt, status='OLD', form='FORMATTED', &
             delim='APOSTROPHE')
      end if
   end if
