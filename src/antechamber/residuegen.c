...
void proceed()
{
    ...
    strcat(command,
           " -o RESIDUE_GEN_RESP.INPUT1 -f resp1 -a RESIDUE_GEN_RESPGEN.DAT -n ");
    ...
    sprintf(tmpchar, "%d", confnum);
    strcat(command,
           " -o RESIDUE_GEN_RESP.INPUT2 -f resp2 -a RESIDUE_GEN_RESPGEN.DAT -n ");
    strcat(command, tmpchar);
    printf("\nRunning: %s\n", command);
    esystem(command);
    fflush(stdout);
    ...
    strcat(command,
           " -O -i RESIDUE_GEN_RESP.INPUT1 -o RESIDUE_GEN_RESP.OUTPUT1 -q QIN -e ");
    ...
    build_exe_path(command, "resp", sizeof command, 1);
    strcat(command,
           " -O -i RESIDUE_GEN_RESP.INPUT2 -o RESIDUE_GEN_RESP.OUTPUT2 -q qout -e ");
    strcat(command, espfile);
    printf("\nRunning: %s", command);
    esystem(command);
    fflush(stdout);
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(COLORTEXT, "YES") == 0 || strcmp(COLORTEXT, "yes") == 0) {
        if (argc == 2 && (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "-H") == 0)) {
            ...
            eprintf("[31mUsage: residuegen [0m input_file.");
        }
    } else {
        if (argc == 2 && (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "-H") == 0)) {
            ...
            eprintf("Usage: residuegen input_file.");
        }
    }
    fpin = efopen(argv[1], "r");
    ...
    for (;;) {
        ...
        if (strncmp("SEP_BOND", line, 8) == 0) {
            sscanf(&line[9], "%s%s%d", sepbond[sepbondnum].resat,
                   sepbond[sepbondnum].capat, &sepbond[sepbondnum].type);
            sepbondnum++;
            continue;
        }
        ...
        if (strncmp("ATOM_CHARGE", line, 11) == 0) {
            if (chargenum >= MAXCHARGE) {
                eprintf("MAXCHARGE is too small in residuegen.c.\n"
                        "Increase MAXCHARGE and rebuild antechamber.");
            }
    ...
    for (i = 0; i < sepbondnum; i++)
        printf("\nSEPBOND\t\t\t%d\t%s\t%s\t%d", i + 1, sepbond[i].resat, sepbond[i].capat,
               sepbond[i].type);
    printf("\nThere are %d predefined charges", chargenum);
    for (i = 0; i < chargenum; i++)
        printf("\nATOM_CHARGE\t\t%d\t%s\t%9.5lf", i + 1, charge[i].name,
               charge[i].charge);
...
/*	read molecule information */
    ...
    overflow_flag = rac(inputfile, &atomnum, atom, &bondnum, bond, &cinfo, &minfo);
    ...
    selectelement2 = (int *) emalloc(sizeof(int) * atomnum);
    flag = (int *) emalloc(sizeof(int) * atomnum);
...
/*	find atom ids for predefined charges*/
    for (i = 0; i < chargenum; i++) {
        ...
        if (index == 0) {
            eprintf("The charge name (%s) is not in the molecule.",
                    charge[i].name);
        }
    }
...
/*	find atom ids for separted bonds*/
    for (i = 0; i < sepbondnum; i++) {
        ...
        for (j = 0; j < atomnum; j++) {
            ...
            if (sepbond[i].resid != -1 && sepbond[i].capid != -1)
                break;
        }
        if (sepbond[i].resid == -1) {
            eprintf("The ATOM_NAME1 (%s) of (%d) is not in the molecule.",
                    sepbond[i].resat, i + 1);
        }
        if (sepbond[i].capid == -1) {
            eprintf("The ATOM_NAME2 (%s) of (%d) is not in the molecule.",
                    sepbond[i].capat, i + 1);
        }
        if (sepbond[i].type == 1) {
    ...
    if (num_sep_bond_type1 > 1) {
        eprintf("Only one or zero SEP_BOND has a type of '1'.");
    }
...
/*
        if (num_terminal_atom != 0 && num_terminal_atom != 2) {
                eprintf("The total number of terminal atoms must be zero or two:\n"
                        "HEAD_ATOM / PSEUDO_HEAD_ATOM + TAIL_ATOM / PSEUDO_TAIL_ATOM.");
        }
*/
/*	find cap atoms and residue atoms*/
    if (sepbondnum > 0) {
        for (i = 0; i < sepbondnum; i++) {
            printf("\n\nCap Atoms for #%d separating bond (%s - %s) with a type of %d",
                   i + 1, sepbond[i].capat, sepbond[i].resat, sepbond[i].type);
            group_atom1(i);
            for (j = 0; j < atomnum; j++)
                if (flag[j] == 1)
...
/*	generate charge input file for respgen */
    printf("\nGenerate charge input file for respgen ... \n");
    fpout = efopen("RESIDUE_GEN_RESPGEN.DAT", "w");
    fprintf(fpout,
            "//predefined charges in a format of (CHARGE partical_charge atom_ID atom_name)");
    for (i = 0; i < chargenum; i++)
        fprintf(fpout, "\nCHARGE        %9.6lf\t%d\t%s", charge[i].charge,
                charge[i].id + 1, charge[i].name);
...
/*	generate main chain file for prepgen */
    ...
    fpout = efopen("RESIDUE_GEN_MAINCHAIN.DAT", "w");
